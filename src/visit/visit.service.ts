...
const INVITATION_SELECT = { id: true, visitorOwnerUserId: true, hostUserId: true, networkCompanionId: true, assetPackId: true, companionName: true, companionDescription: true, companionTags: true, status: true, expiresAt: true, respondedAt: true, cancelledAt: true, createdAt: true, updatedAt: true } as const;
const SESSION_SELECT = { id: true, invitationId: true, visitorOwnerUserId: true, hostUserId: true, networkCompanionId: true, assetPackId: true, state: true, visitorOwnerReadyAt: true, hostReadyAt: true, readyAt: true, startedAt: true, endedAt: true, endReason: true, failureCode: true, createdAt: true, updatedAt: true } as const;
...
export class VisitService implements OnModuleInit, OnModuleDestroy {
  ...
  async createInvitation(visitorOwnerUserId: string, hostUserId: string) {
    ...
    const snapshot = await this.loadCurrentSnapshot(visitorOwnerUserId);
    ...
    const invitation = await this.prisma.$transaction(async tx => {
      ...
      return tx.visitInvitation.create({ data: {
        visitorOwnerUserId, hostUserId, networkCompanionId: snapshot.companion.id, assetPackId: snapshot.pack.id,
  ...
  async acceptInvitation(hostUserId: string, invitationId: string) {
    const result = await this.prisma.$transaction(async tx => {
      ...
      if (invitation.expiresAt <= new Date()) {
        return { invitation: await tx.visitInvitation.update({ where: { id: invitation.id }, data: { status: 'expired', respondedAt: new Date() }, select: INVITATION_SELECT }), expired: true };
      }
      ...
      const pack = await tx.companionAssetPack.findUnique({ where: { id: invitation.assetPackId }, select: { id: true, companionId: true, status: true } });
      if (!pack || pack.companionId !== invitation.networkCompanionId || !['active', 'superseded'].includes(pack.status)) this.notAvailable();
      const accepted = await tx.visitInvitation.update({ where: { id: invitation.id }, data: { status: 'accepted', respondedAt: new Date() }, select: INVITATION_SELECT });
      const session = await tx.visitSession.create({ data: {
        ...
        networkCompanionId: invitation.networkCompanionId, assetPackId: invitation.assetPackId, state: 'preparing',
  ...
  async endSessionsBetween(userA: string, userB: string, reason: string) {
    ...
    for (const session of sessions) {
      const claimed = await this.prisma.visitSession.updateMany({ where: { id: session.id, state: session.state }, data: { state: session.state === 'active' ? 'ended' : 'cancelled', endedAt: new Date(), endReason: reason } });
  ...
  async cleanupExpiredAndTimedOut(limit = 500) {
    ...
    try {
      ...
      for (const invitation of expired) {
        const updated = await this.prisma.visitInvitation.updateMany({ where: { id: invitation.id, status: PENDING, expiresAt: { lt: now } }, data: { status: 'expired', respondedAt: now } });
        if (updated.count) {
      ...
      for (const session of sessions) {
        ...
        const updated = await this.prisma.visitSession.updateMany({ where: { id: session.id, state: session.state }, data: { state, endedAt: now, endReason: reason } });
  ...
  private async respondToInvitation(userId: string, invitationId: string, role: 'host' | 'owner', status: 'declined' | 'cancelled') {
    const invitation = await this.prisma.$transaction(async tx => {
      ...
      return tx.visitInvitation.update({ where: { id: current.id }, data: status === 'declined' ? { status, respondedAt: new Date() } : { status, cancelledAt: new Date() }, select: INVITATION_SELECT });
    });
  ...
  private async endSessionInTransaction(tx: Prisma.TransactionClient, sessionId: string, userId: string, requestedReason?: string) {
    ...
    return tx.visitSession.update({ where: { id: current.id }, data: { state: current.state === 'preparing' || current.state === 'ready' ? 'cancelled' : 'ended', endingAt: current.state === 'active' || current.state === 'ending' ? new Date() : null, endedAt: new Date(), endReason: reason }, select: SESSION_SELECT });
  }

  private async authorizeSessionAsset(hostUserId: string, sessionId: string): Promise<any> {
    this.requireFeature();
    const session = await this.prisma.visitSession.findUnique({ where: { id: sessionId }, select: { id: true, hostUserId: true, visitorOwnerUserId: true, assetPackId: true, networkCompanionId: true, state: true } });
    ...
    const pack = await this.prisma.companionAssetPack.findUnique({ where: { id: session.assetPackId }, include: { files: true } });
    if (!pack || pack.companionId !== session.networkCompanionId || !['active', 'superseded'].includes(pack.status)) this.assetNotAvailable();
    return pack;
  }
  ...
  private async loadCurrentSnapshot(ownerId: string): Promise<any | undefined> {
    const user = await this.prisma.user.findUnique({ where: { id: ownerId }, select: { activeNetworkCompanion: { include: { activeAssetPack: true } } } });
    const companion = user?.activeNetworkCompanion;
    const pack = companion?.activeAssetPack;
    if (!companion || !pack || !companion.published || companion.visibility !== 'friends_only' || companion.activeAssetPackId !== pack.id || pack.status !== 'active') return undefined;
  ...
  private timeoutReason(session: any, now: Date): string | undefined {
    ...
    if ((session.visitorOwnerSeenAt?.getTime() ?? 0) < threshold || (session.hostSeenAt?.getTime() ?? 0) < threshold) return 'heartbeat_timeout';
    return undefined;
  }
