...
describe('ProtocolConfigService', () => {
  ...
  it('uses one configured protocol source for compatibility decisions', () => {
    const service = new ProtocolConfigService(config({ PROTOCOL_VERSION: '0.2', MINIMUM_CLIENT_VERSION: '1.4.0', SERVER_VERSION: '7.0.0' }) as never);
    expect(service.protocolVersion).toBe('0.2');
    expect(service.minimumClientVersion).toBe('1.4.0');
    expect(service.isCompatible('1.4.0', '0.2')).toEqual({ compatible: true });
    expect(service.isCompatible('1.3.9', '0.2')).toEqual({ compatible: false, reason: 'CLIENT_VERSION_TOO_OLD' });
